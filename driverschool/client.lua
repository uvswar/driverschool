...
local function StartDriveTest(type)
	...
	QBCore.Functions.Notify(Lang:t('info.seatbelt'), 'info', 4000)
...
Citizen.CreateThread(function()
	while true do 
		if testRunning then
			...
			local cancelCoords = vector3(248.95, -1397.02, 30.52)
			local distance = #(ped - cancelCoords)
			-- print(distance)
			if distance < 3 then 
				DrawText3Ds(cancelCoords, 'Press [E] to cancel Driving Test')
